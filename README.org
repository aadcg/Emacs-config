...
(operating-system
 ...
 ;; I like to use the latest longterm release (aka "stable")
...
You can get substitutes for non-guix from https://mirror.brielmaier.net.
...
(use-package slime
  ...
  (slime-completion-at-point-functions '(slime-filename-completion slime-simple-completion-at-point))
  ;; (slime-port 4006)
  :config
...
- [[https://github.com/ch11ng/exwm/wiki#how-to-send-c-c-to-term][how to copy/paste in firefox? C-c is intercepted even in char-mode,
  why?]]
- send patch to exwm-edit to handle (exwm-input--fake-key ?\C-a)
  (sometimes C-a isn't the right key to copy the contents) ([[https://askubuntu.com/questions/294953/gtk-key-theme-emacs-what-keystroke-is-for-select-all][see]])
...
;;   :init (global-exwm-edit-mode)
;;   :custom (exwm-edit-split "below")
;;   :hook (exwm-edit-compose-hook . org-mode))
...
- external-editor-program or exwm-edit in nyxt?
- how to unbound a keybinding?
- add eww keybindings

[[https://github.com/jmercouris/configuration/blob/master/.config/nyxt/init.lisp][jmercouris config]]
...
(define-key *my-keymap*
  ...
  ;; first I need to figure out vi:normal/insert mode in nyxt
...
;; auto-mode could easily replace this
...
* TODO
** say goodbye to use-package
replace bind-keys with define-key
(info "(elisp) Remapping Commands")

read about package.el

** use gpg as ssh key

** [[file:~/repos/prot-dotfiles/emacs/.emacs.d/emacs-init.org::#h:3d8ebbb1-f749-412e-9c72-5d65f48d5957][Window rules and basic tweaks]]
(info "(emacs) Buffers")
...
** dumb-jump
so that M-. works for all projects?
