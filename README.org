...
* Ethos
I started using GNU Emacs in August 2018, and my life changed forever.
[[https://www.youtube.com/watch?v=FLjbKuoBlXs&t=0][Protesilaos Stavrou argues whether this tool is right for you]].

/Are you ready to go down the rabbit hole?/

[[https://www.gnu.org/philosophy/philosophy.html][GNU is about freedom]].

For more information about the GNU project and its founder, Richard
...
** GNU Guix
Valuable information can be found at the help mailing lists or the [[http://logs.guix.gnu.org/guix/][IRC
Logs]].

Some man pages of system packages aren't available so you need to issue:
...
(use-package dired
  ...
  (dired-listing-switches "-Athl --group-directories-first")
...
** Projects
*** Version Control
Email isn't set globally since it depends on the project.
...
(use-package magit
  ...
  :config
  (defun git-set-email (email)
    "Set email on the current git project."
    (interactive "sEmail: ")
...
How could I get auto-indentation on inferior python?  There could be an
ipython minor mode on top of interactive python. [[https://lists.gnu.org/archive/html/emacs-devel/2014-08/msg00448.html][Emacs Devel Thread]]

I highly recommend reading the [[https://ipython.readthedocs.io/en/stable/][IPython documentation]].
...
| %run -d /path   | debug               |              |
| %run -p /path   | profile             |              |
| %pastebin #line |                     | %pastebin _i |
| %xmode          | set the debug level |              |
| !any-shell-cmd  |                     |              |
| _               | last output         |              |
| _i<n>           | last nth input      |              |
| _<n>            | last nth output     |              |
...
#+begin_src python :tangle ~/.ipython/profile_default/startup/init.py
print("""\n\"When you start Lisp, it does 'read', 'eval', and 'print', all
of which are missing in Python.\" --- Richard Stallman\n\nHappy Hacking!""")
#+end_src
...
(use-package python
  ...
  (python-shell-interpreter-args
   (mapconcat
    'identity
    '("-i" "--simple-prompt" "--no-automagic" "--pdb" "--classic"
...
(use-package elpy
  :init (elpy-enable)
  :config
  (defun my-elpy-module-eldoc (command &rest _args)
  ...
  (defalias 'elpy-module-eldoc 'my-elpy-module-eldoc)
  :custom
...
** Macros
For text use org macros.  Within source code blocks, use noweb syntax.

** Deprecate ssh agency
I had an issue.  See [[https://stackoverflow.com/questions/60812866/emacs-gpg-pinentry-el-for-authentication][this]].  [[https://www.youtube.com/watch?v=4-Ks_f8rQFA][Tropin's video]] is good.

** Owncloud
If you start it from $HOME, then have ~/etc/ownCloud/sync-exclude.lst.
Why?  Хз.

** Emacs loads deleted packages
E.g. company.

** DPI
[[https://linuxreviews.org/HOWTO_set_DPI_in_Xorg#How_DPI_Values_Are_Used][How to config]]

External monitor - 118
Internal monitor - 282 (at 3200x1800)
                 - 226 (at 2560x1440)
