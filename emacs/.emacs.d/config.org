...
** Termux
I do. I have the following predicate in my init.el (think it is from Sacha Chua)
...
Then for any package, that I do not want installed in termux, I start the use-package block with:

    (use-package pkgname
      :unless in-termux-p

I use straight.el, so getting my config working on termux was pretty much just: git clone dotfiles, cd dotfiles, stow emacs
