...
#+begin_src emacs-lisp
  ...
  (defun my-get-root-ru-word ()
    ...
    (let* ((spelling-sentence (s-split-words (my-ispell-output)))
           (len (length spelling-sentence)))
      (if (> len 3)
          (nth (1- len) spelling-sentence)
        (nth 0 spelling-sentence))))
#+end_src
